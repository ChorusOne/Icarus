    ...
    UsingPrecompiles
{
    ...
    function setApprovalStageDuration(uint256 approvalStageDuration)
        public
        onlyOwner
    {
        require(approvalStageDuration > 0, "Duration must be larger than 0");
        require(
            approvalStageDuration != stageDurations.approval,
            "Duration unchanged"
        );
    ...
    function removeIfQueuedAndExpired(uint256 proposalId)
        ...
        returns (bool)
    {
        bool expired = queue.contains(proposalId) &&
            isQueuedProposalExpired(proposalId);
        if (expired) {
        ...
        returns (bool)
    {
        ...
        ) = requireDequeuedAndDeleteExpired(proposalId, index);
        if (!proposal.exists()) {
            return false;
        }
    ...
    function isQueued(uint256 proposalId) public view returns (bool) {
        return
            queue.contains(proposalId) && !isQueuedProposalExpired(proposalId);
    }
        ...
        returns (bool)
    {
        ...
        return
            _isDequeuedProposalExpired(
                proposal,
                proposal.getDequeuedStage(stageDurations)
            );
    }
